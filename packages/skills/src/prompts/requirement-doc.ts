...
export const REQ_PROMPT = (
  ...
  // 关键：当用户贴了文档时，文档是权威源，群聊记录只能作为「补充上下文」用。
  // 但「多份文档时必须全部融合」是另一条强约束 —— 用户经常分多次贴：第 1 份是
  // 主 PRD，第 2 份是补充背景/DDL/范围调整。LLM 容易误判为「不同项目，挑一个」，
  // 这里必须明确说「都是同一项目」，不许只挑一份。
  const priorityDirective = hasDocs
    ? `本次输入中包含 ${linkedDocs.length} 份关联文档（见下方「关联文档」段落）。

**关键规则——多份文档必须融合，不许只挑一份：**
- 用户贴的多份关联文档**默认都是描述同一个项目**的不同方面：第 1 份可能是主 PRD，
  第 2 份可能是补充背景 / DDL / 范围调整 / 海外市场扩展等。
- title / background / goals / scope / deliverables 必须**综合所有 ${linkedDocs.length} 份文档的全部信息**，
  尤其要把「补充」「附加」「扩展」「DDL」「时间约束」类的信息都体现在 background / scope / goals 里。
- 如果文档之间有矛盾（如范围调整），**以最新 / 更具体的那份为准**，但**两份都要在最终 PRD 中
  体现影响**。
- 仅在两份文档明显是**完全不同的项目**（标题、目标、领域都对不上）时才独立处理，
  这种情况非常罕见。

群聊记录仅在文档明确缺失某字段时作为补充参考。`
