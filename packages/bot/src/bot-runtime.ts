    ...
    try {
      const dispatcher = new lark.EventDispatcher({
        ...
        'im.message.receive_v1': async (data) => {
          // 诊断日志：长连接送来的每一条 message 事件，先记录原始指纹，
          // 再尝试解析。任何 parse 失败也能在 log 里看到，避免事件被静默吞掉。
          try {
            const raw = data as unknown as Record<string, unknown>;
            const m = (raw['message'] ?? {}) as Record<string, unknown>;
            const s = (raw['sender'] ?? {}) as Record<string, unknown>;
            console.info(
              `[BotRuntime] im.message.receive_v1 message_id=${String(m['message_id'] ?? '?')} chat_id=${String(m['chat_id'] ?? '?')} chat_type=${String(m['chat_type'] ?? '?')} message_type=${String(m['message_type'] ?? '?')} sender_id_type=${String(((s['sender_id'] ?? {}) as Record<string, unknown>)['sender_type'] ?? '?')}`,
            );
          } catch (e) {
            console.error('[BotRuntime] failed to log raw message event', e);
          }
          try {
            const msg = parseMessage(data as unknown as Record<string, unknown>);
            this.emit({ type: 'message', payload: msg });
          } catch (e) {
            console.error('[BotRuntime] parseMessage threw, event dropped:', e);
          }
          return { code: 0 };
        },
  ...
  async fetchMessage(
    ...
    try {
      ...
      const messages: Message[] = items.map((item) => {
        ...
        const senderOpenId =
          typeof sender.id === 'string' ? (sender.id as string) : (senderId.open_id as string | undefined);
        return parseMessage({
          message: {
            message_id: item.message_id,
            chat_id: item.chat_id,
