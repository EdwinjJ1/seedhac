...
async function main(): Promise<void> {
  ...
  await send(
    '1/10 activation — 激活确认',
    larkCardBuilder.build('activation', {
  ...
  await send(
    '3/10 tablePush — 分工表推送',
    larkCardBuilder.build('tablePush', {
      tableTitle: '业务探索 · 分工表',
  ...
  await send(
    '8/10 offlineSummary — 离线摘要',
    larkCardBuilder.build('offlineSummary', {
  ...
  await send(
    '9/10 docChange — 文档变更通知',
    larkCardBuilder.build('docChange', {
      editorName: '沛彤',
      docTitle: '业务探索需求文档',
      docUrl: 'https://example.feishu.cn/docs/req',
